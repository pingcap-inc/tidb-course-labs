...
def _non_batch_style_insert(cursor, row_count):
    ...
    for r in range(1, row_count + 1):
        cursor.execute(sql_insert, (str(r),))
...
def _check_result(cursor):
    query = "SELECT count(*) as row_count FROM t1_batchtest"
    cursor.execute(query)
    result = cursor.fetchone()
    print(f"Total rows in t1_batchtest table: {result['row_count']}.")


if __name__ == "__main__":
    with get_connection() as connection:
        print(f"Connected to TiDB: {connection.user}@{connection.host}:{connection.port}")
