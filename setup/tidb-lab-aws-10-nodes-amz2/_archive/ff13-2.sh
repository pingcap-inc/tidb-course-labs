...
tiup cdc:v7.5.1 cli changefeed create --pd=http://${HOST_PD1_PRIVATE_IP}:2379 \
--sink-uri="mysql://cdc_user:q1w2e3R4_@${HOST_PD1_PRIVATE_IP}:3306/" \
--changefeed-id="replication-task-1" --sort-engine="unified"

tiup cdc:v7.5.1 cli changefeed list --pd=http://${HOST_PD1_PRIVATE_IP}:2379

tiup cdc:v7.5.1 cli changefeed query --pd=http://${HOST_PD3_PRIVATE_IP}:2379 --changefeed-id=replication-task-1 
