...
~/.tiup/bin/tiup ctl:v7.5.1 cdc capture list --pd=http://${HOST_PD1_PRIVATE_IP}:2379
...
~/.tiup/bin/tiup cdc:v7.5.1 cli changefeed create --pd=http://${HOST_PD1_PRIVATE_IP}:2379 \
--sink-uri="mysql://cdc_user:q1w2e3R4_@${HOST_PD1_PRIVATE_IP}:3306/" \
--changefeed-id="replication-task-1" --sort-engine="unified"

~/.tiup/bin/tiup cdc:v7.5.1 cli changefeed list --pd=http://${HOST_PD1_PRIVATE_IP}:2379

~/.tiup/bin/tiup cdc:v7.5.1 cli changefeed query --pd=http://${HOST_PD3_PRIVATE_IP}:2379 --changefeed-id=replication-task-1 
...
# 13-3
#~/.tiup/bin/tiup cdc:v7.5.1 cli changefeed pause --pd=http://${HOST_PD1_PRIVATE_IP}:2379  --changefeed-id replication-task-1
#~/.tiup/bin/tiup cdc:v7.5.1 cli changefeed remove --pd=http://${HOST_PD1_PRIVATE_IP}:2379 --changefeed-id replication-task-1
#~/.tiup/bin/tiup cdc:v7.5.1 cli changefeed list --pd=http://${HOST_PD3_PRIVATE_IP}:2379
